...
public class Skills {
     ...
     * @param skill The skill.
     */
    public void sendLevelUpMessage(int skill) {
        StatFrames frames = StatFrames.values()[skill];

        String message1 = "Congratulations! You've just advanced a " + SKILL_NAME[skill].toLowerCase() + " level!";
        String message2 = "You have now reached level " + getLevelForExperience(skill) + ".";

        mob.getActionSender().removeInterface();
        mob.getActionSender().sendMessage(message1);
        mob.getActionSender().sendMessage(message2);

        mob.getActionSender().sendString(740, 0, message1);
        mob.getActionSender().sendString(741, 1, message2);

        mob.getActionSender().sendChatInterface(frames.getFrame());
    }
    ...
    private boolean messageMatchesPattern(int skill) {
        return !StatFrames.RANGED.equals(skill)
                || !StatFrames.MINING.equals(skill)
                || !StatFrames.THIEVING.equals(skill)
                || !StatFrames.FARMING.equals(skill);
    }
}
