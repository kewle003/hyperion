...
public class ItemDefinition {
    ...
    public static void init() throws IOException {
        ...
        RandomAccessFile raf = new RandomAccessFile(Constants.ITEM_DEFINITIONS_FILE, "r");
        try {
...
//
            //    int id = 10896;
            //	definitions[id] = new ItemDefinition(id, "Wolf Whistle", "A whistle to call out wolfs.", false, false, false, id, id, true, 100);
     ...
     * @throws IllegalStateException if the definitions have been loaded already.
     */
    public static void dump(boolean sql) {
        try {
            FileWriter fstream = new FileWriter("data/itemDefinitions.sql");
            ...
            if (sql) {
                out.write("INSERT INTO `item_definitions` (`id`, `name`, `examine`, `noted`, `notable`, `stackable`, `parentid`, `notedId`,`members`,`shopValue`) VALUES ");
            }
            for (int i = 0; i < definitions.length; i++) {
                ...
                if (item != null) {
                    buf.putShort((short) item.getId());
                    if (sql) {
                        out.write("('" + item.getId() + "', '" + item.getName().replace("'", "") + "', '" + item.getDescription().replace("'", "") + "', '" + (item.isNoted() ? 1 : 0) + "', '" + item.getNormalId() + "', '" + (item.isNoteable() ? 1 : 0) + "', '" + item.getNotedId() + "', '" + (item.isStackable() ? 1 : 0) + "', '" + (item.isMembersOnly() ? 1 : 0) + "', '" + item.getValue() + "'),\n");
                    }
