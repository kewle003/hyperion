...
public class PrivateChat {
	...
	/**
	 * @param player the player to setQuestStage
	 */
	...
	/**
	 * @param channelName the channelName to setQuestStage
	 */
		...
		if(player != null) {
			...
			if(p != null) {
				...
				if(p.getPrivateChat().getFriends().containsKey(player.getNameAsLong())
								&& player.getInterfaceState().getPrivateChat() == 1) { //if we've added them, but we had private setQuestStage to 'friends', we need to tell them we are now online because we are friends with them
	...
	/**
	 * @param friends the friends to setQuestStage
	 */
	...
	public void removeIgnore(long nameAsLong) {
		...
		if(p != null) {
			if (player.getInterfaceState().getPrivateChat() != 2 //no point telling the client again if we are already offline
							...
							&& !(player.getInterfaceState().getPrivateChat() == 1 && !friends.containsKey(p.getNameAsLong()))) { //don't send offline if we already have private setQuestStage to 'friends' and they aren't our friend
				p.getActionSender().sendFriend(player.getNameAsLong(), 1, p.getPrivateChat().getFriends().get(player.getNameAsLong()).getId());
			}			
		}
	}
	...
	/**
	 * @param ignores the ignores to setQuestStage
	 */
	...
	/**
	 * @param entryRank the entryRank to setQuestStage
	 */
	...
	/**
	 * @param talkRank the talkRank to setQuestStage
	 */
	...
	/**
	 * @param kickRank the kickRank to setQuestStage
	 */
