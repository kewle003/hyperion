...
public class FightPits extends AbstractMinigame {
	...
	public void quit(final Player player) {
		...
		} else if(getPlayer().contains(player)) {
			player.resetVariousInformation();
			getPlayer().remove(player);
			if(getPlayer().size() == 1) { //one guy left
				...
				for(Player participant : getPlayer()) {
					participant.getActionSender().sendString(373, 1, "Foes Remaining: " + (getPlayer().size() - 1));
				}
			}
		}
			...
			public void execute() {
				for(Player player : participants) {
					player.getActionSender().sendString(373, 0, "Current Champion: " + getWinner())
											.sendString(373, 1, "Foes Remaining: " + (getPlayer().size() - 1));
	...
	public void killHook(Player player, Mob victim) {
		...
		if(victim.isPlayer()) {
			...
			if(getPlayer().contains(pVictim)) {
				quit(pVictim);
			}
		}
		if(getPlayer().contains(player) && getPlayer().size() <= 1) { //only us left
			winGame(player);
		} else {
			for(Player participant : getPlayer()) {
				participant.getActionSender().sendString(373, 1, "Foes Remaining: " + (getPlayer().size() - 1));
