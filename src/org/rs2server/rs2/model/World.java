...
import org.rs2server.rs2.event.impl.UpdateEvent;
...
public class World {
    ...
    private void loadConfiguration() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        try {
            ...
            if (mappings.containsKey("worldLoader")) {
                ...
                logger.fine("WorldLoader setQuestStage to : " + worldLoaderClass);
            } else {
                this.loader = new GenericWorldLoader();
                logger.fine("WorldLoader setQuestStage to default");
            }
    ...
    public void register(GameObject obj) {
        ...
        obj.getRegion().getGameObjects().add(obj);   //TODO: check this out leech
    }
    ...
    public void replaceObject(final GameObject original, final GameObject replacement, int cycles) {
        ...
        submit(new Tickable(cycles) {
            ...
            public void execute() {
                ...
                //TODO: Cheap fix, loaded in landscape is setQuestStage to false
                register(new GameObject(original.getLocation(), original.getId(), original.getType(), original.getDirection(), false));
                this.stop();
            }
        });
    }
    ...
    public void unregister(final Player player) {
        if (player.isActive()) {
            ...
            if (player.getCutScene() != null) {
                player.getCutScene().stop(player);
            }
            ...
            if (player.getQuestStorage().getQuestStage(new WerewolfQuest()) == 2
                    || player.getQuestStorage().getQuestStage(new WerewolfQuest()) == 3) {
                player.getQuestStorage().setQuestStage(new WerewolfQuest(), 1);
                player.setLocation(Location.create(player.getLocation().getX(), player.getLocation().getY(), 0));
            }
