...
public class EquipmentDefinition {
	...
	/**
	 * @param definitions the definitions to setQuestStage
	 */
	...
	public static void init() throws IOException {
		...
		try {
			...
			File file = new File(Constants.EQUIPMENT_DEFINITIONS_FILE);
			if(file.exists()) {
		...
		/*for(ItemDefinition itemDef : ItemDefinition.getDefinitions()) {
			...
			EquipmentDefinition def = definitions.get(itemDef.getId());
			if(def != null) {
				if(itemDef.getId() == 4734) {
					definitions.put(itemDef.getId(), new EquipmentDefinition(3, def.getBonuses(), def.getWeaponStyles(),
									def.getType(), def.getAnimation(), def.getPoisonType(), def.getSkillRequirements(), def.getSpecialConsumption(), def.getSpecialHits()));
