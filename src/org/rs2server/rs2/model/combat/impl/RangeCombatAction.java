        ...
        if (pullback == null && projectile == -1 && attacker.getEquipment() != null) {
            ...
            if (bowType != null) { //standard bow and arrow
                ...
                if (bowType == BowType.CRYSTAL_BOW) {
                    ...
                    attacker.getCombatState().setBonus(12, arrowEquipDef.getBonus(12));//we are using arrows, so we must setQuestStage to use the arrows' ranged strength
                    ammunition = arrows;
                }
                if (bowType == BowType.DARK_BOW) {
                ...
                if (rangeWeaponType != null) {
                    attacker.getCombatState().setBonus(12, weaponEquipDef.getBonus(12));//we are using a range weapon, so we must setQuestStage to use the weapon's ranged strength
                    ammunition = weapon;
