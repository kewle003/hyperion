...
public class Region {
	...
	public Door doorForLocation(Location location, int id) {
		for(Door door : doors) {
			if(door.getObject().getLocation().equals(location) && door.getObject().getId() == id || door.getReplacement().getLocation().equals(location) && door.getReplacement().getId() == id) {
				return door;
			}
		}
		return null;
