...
public class ActionButtonPacketHandler implements PacketHandler {
    ...
    public void handle(final Player player, Packet packet) {
        ...
        final Quest quest = QuestRepository.get(button);
        ...
        if (quest != null) {
            quest.actionButtons(player, interfaceId, childButton, button);
        }
        switch (interfaceId) {
            ...
            case 274:
                if (quest != null) {
                    if (button == quest.getQuestButton()) {
                        quest.getProgress(player, true, player.getQuestStorage().getQuestStage(quest));
                        player.getActionSender().sendString(275, 2, quest.getQuestName());
                    }
                    return;
                } else {
                    player.getActionSender().removeInterface();
                    player.getActionSender().sendMessage("This quest has not been added yet!");
                }
                break;
            /*
            * Login screen
            ...
            case 378:
                switch (button) {
                    case 6:
                        ...
                        if (!f.exists()) {
                            player.setCutScene(Starter.getScene());
                            player.getCutScene().start(player);
                        }
                        return;
                }
                return;
                    ...
                    public void execute() {
                        ...
                        if (emote != null) {
                            if (emote.getAnimation() != null) {
                                if (emote.getAnimation() == Animation.DANCE &&
                                        player.getEquipment().contains(15446)
                                        || player.getEquipment().contains(10742)) {
                                    player.playAnimation(Animation.FLARED_DANCE);
                                } else if (emote.getAnimation() == Animation.SALUTE &&
                                        player.getEquipment().contains(10744)) {
                                    player.playAnimation(Animation.ENHANCED_SALUTE);
                                } else if (emote.getAnimation() == Animation.YAWN &&
                                        player.getEquipment().contains(10746)) {
                                    player.playAnimation(Animation.ENHANCED_YAWN);
                                } else if (emote.getAnimation() == Animation.ANGRY &&
                                        player.getEquipment().contains(10740)) {
                                    player.playAnimation(Animation.ENHANCED_ANGRY);
                                }
