...
public class ActionSender {
    ...
    public ActionSender sendLogin() {
        ...
        System.out.println(player.getName() + " Rights: " + player.getRights() + " Member: " + player.isMembers() + " |Days: " + player.getDaysOfMembership());
        ...
        sendString(550, 2, "Friends List - " + Constants.SERVER_NAME);
        sendQuestTab();
     ...
     * Sends a debug message.
     *
     * @param description The message to send.
     * @return The action sender instance, for chaining.
     */
    ...
    /**
     * Sends a multiple strings.
     *
     * @param interfaceId The interface id.
     * @param string      The multiple strings.
     * @return The action sender instance, for chaining.
     */
    public ActionSender sendString(int interfaceId, int child, String... string) {
        PacketBuilder bldr = new PacketBuilder(231, Type.VARIABLE_SHORT);
        ...
        for (int i = 0; i < string.length; i++) {
            sendString(interfaceId, 0 + i, string[i]);
        }
        player.write(bldr.toPacket());
        return this;
    }
...
//				player.getRequestManager().setState(RequestState.NORMAL);
    ...
    /**
     * Sends an info dialogue (like the one from tutorial island, "getting started")
     *
     * @param dialogueType The message type.
     * @param item         The item.
     * @param text         The message(s)
     * @return The action sender instance, for chaining.
     */
    public ActionSender sendInformationBox(DialogueType dialogueType, int item, String... text) {
        int interfaceId = -1;
        switch (dialogueType) {
            case MESSAGE:
                interfaceId = 219;
            ...
            case MESSAGE_MODEL_LEFT:
                ...
                sendInterfaceModel(519, 0, 130, item);
                sendChatboxInterface(interfaceId);
                break;
            default:
                break;
        ...
        for (long l : player.getPrivateChat().getFriends().keySet()) {
            if (World.getWorld().getPlayerNames().get(l) != null) {
                ...
                if (p.getPrivateChat().getFriends().containsKey(player.getNameAsLong())
                        && player.getInterfaceState().getPrivateChat() == 1) { //if we've added them, but we had private setto 'friends', we need to tell them we are now online because we are friends with them
    ...
    public ActionSender sendAreaInterface(Location before, Location after) {
        ...
        if (afterInPvP) {
            if (!afterInFightPits) {
                ...
                sendString(373, 1, World.getWorld().getFightPits().getPlayer().size() > 1 ? "Foes Remaining: " + (World.getWorld().getFightPits().getPlayer().size() - 1) : "You're the Winner!");
            }
            if (!beforeInPvP) {
                if (afterInFightPits) {
